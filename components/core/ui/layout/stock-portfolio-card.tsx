...
const NewsItem: React.FC<{ article: NewsArticle }> = ({ article }) => (
	<div className="flex-shrink-0 w-[220px] p-4 bg-muted/50 rounded-lg">
		...
		<p className="font-semibold text-sm text-card-foreground leading-snug mb-3">
			{article.title}
		</p>
		<a
			href="#"
...
export const StockPortfolioCard = ({
	...
	return (
		<motion.div
			...
			animate="visible"
			className={cn(
				"w-full max-w-2xl mx-auto rounded-xl border bg-card text-card-foreground shadow-sm p-6 space-y-6",
				className,
			)}
		>
			{/* Header Section */}
			...
			<motion.div variants={itemVariants}>
				<div className="flex justify-between items-center mb-2">
					...
					<div className="flex gap-2">
						<button className="p-1 rounded-full border bg-background hover:bg-muted">
