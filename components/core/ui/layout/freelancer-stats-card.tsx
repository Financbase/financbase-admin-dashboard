...
>(
	(
		{ title, timeFrame, earnings, subStats, ranking, availability, className },
		ref,
	) => {
		...
		return (
			...
			>
				...
				<header className="flex justify-between items-center">
					<h2 id="stats-card-title" className="text-xl font-bold">
						{title}
					</h2>
				...
				<section
					...
					aria-label="Projects and Clients"
				>
				...
				<section aria-labelledby="availability-title">
					<h4 id="availability-title" className="text-md font-semibold">
						{availability.title}
					</h4>
					<motion.div
					...
					>
						{availability.bars.map((bar, index) => {
							...
							const colorIndex = Math.floor(
								(index / availability.bars.length) * (barColors.length - 1),
							);
							const color =
								bar.level > 0 ? barColors[colorIndex] : "hsl(var(--muted))";

							return (
								<div
									key={`bar-${index}-${bar.level}`}
