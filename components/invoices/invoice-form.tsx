...
export function InvoiceForm({ initialData, invoiceId, onCancel }: InvoiceFormProps) {
	...
	const { submit, isLoading, error, errors } = useFormSubmission(
	...
	const handleSubmit = async (e: React.FormEvent, action: 'save' | 'send' = 'save') => {
		...
		// Prepare data for API - transform to match API schema
		const apiData = {
			...formData,
			...
			// Generate invoice number if not provided (will be generated by API if missing)
		...
		<div className="space-y-6">
			...
			{Object.keys(errors).length > 0 && (
				<Card className="border-yellow-200 bg-yellow-50 dark:border-yellow-800 dark:bg-yellow-950">
					<CardContent className="pt-6">
						<p className="text-sm font-medium text-yellow-800 dark:text-yellow-200 mb-2">Please fix the following errors:</p>
