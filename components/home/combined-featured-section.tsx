...
// ----------------- Map -------------------
const dottedMapInstance = new DottedMap({ height: 55, grid: 'diagonal' })
  ...
  return (
    <ChartContainer className="h-60 aspect-auto" config={chartConfig}>
      <AreaChart data={chartData}>
        ...
        <Area strokeWidth={2} dataKey="mobile" type="monotone" fill={`url(#${mobileGradientId})`} stroke="var(--color-mobile)" />
        <Area strokeWidth={2} dataKey="desktop" type="monotone" fill={`url(#${desktopGradientId})`} stroke="var(--color-desktop)" />
      </AreaChart>
    </ChartContainer>
  )
}
...
`,
  ...
  // dangerouslySetInnerHTML is required here for dynamic CSS variable injection
  // based on chart configuration themes. The content is generated from controlled
  // configuration data, not user input.
...
const ChartTooltipContent = React.forwardRef<
  HTMLDivElement,
  {
    ...
    label?: React.ReactNode
    labelFormatter?: (label: React.ReactNode, payload: Array<TooltipPayload>) => React.ReactNode
    labelClassName?: string
    formatter?: (value: number | string, name: string, item: TooltipPayload, index: number, payload: Record<string, unknown>) => React.ReactNode
    color?: string
    hideLabel?: boolean
...
function getPayloadConfigFromPayload(
  config: ChartConfig,
  payload: TooltipPayload | Record<string, unknown>,
  key: string,
) {
