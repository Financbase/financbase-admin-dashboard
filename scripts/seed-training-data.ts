 ...
 * Run with: pnpm tsx scripts/seed-training-data.ts
 */
	...
	try {
		...
		for (const program of trainingPrograms) {
			...
			if (result && result.length > 0) {
				...
				// Program already exists, fetch its ID
		...
		const learningPaths = [
			...
			},
			{
				...
				duration: "1 hour",
				icon: "Target",
				programIds: [programIds[0], programIds[1], programIds[3]], // Getting Started, Dashboard, Expense Tracking
			},
		];
